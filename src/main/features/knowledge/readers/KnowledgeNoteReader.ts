 ...
 *
 * The snapshot is read verbatim — not through the markdown reader, whose lossy
 * transforms would break the file text -> canonical `content.text` round-trip the
 * snapshot exists for: the stored index can then be reconciled against the file
 * by content hash instead of re-embedding (matching the url reader). The note
 * snapshot carries no frontmatter, so it is read as-is.
 */
