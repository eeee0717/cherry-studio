...
Migrated v1 folders are tombstones: a v1-indexed `directory` item migrates as `failed` + the `directory_not_migrated` code, which the UI renders as an amber re-embed warning ("needs re-embed") instead of a red error. Re-indexing it runs the normal expansion from `data.path` (copy files into the base, create child items, embed). See §7 for the decision record.
...
- **v1 directory migration — tombstone (decided 2026-06-11)**: a v1-indexed folder migrates as `failed` + `directory_not_migrated` (UI: amber re-embed warning); re-indexing re-expands from `data.path`. Background: v1 embedded each folder file individually but booked every loader id on the one directory item (no per-file items), so the vector migrator drops those vectors as `non_indexable_container`. Alternatives rejected: (a) re-attributing vectors via the legacy `vectors.source` column into synthesized child file items — the data is sufficient (source = file path per chunk row), but the migrator coordination (child-item synthesis across both migrators, name dedupe, file backfill from a possibly-missing external disk) wasn't worth it for a user-recoverable state; (b) placeholder files reconstructed from chunk text — any later reindex or watcher event would bake the synthetic content in as truth, and binary extensions (`.pdf` etc.) cannot host text placeholders. The legacy embedjs DB stays in place (rollback insurance), so a recovery tool that regroups directory vectors by `source` remains possible — it must be decided before any v1-leftover cleanup runs.
- **Watcher preconditions (surfaced by the directory-migration review)** — constraints for the future file-watcher PR:
  - Index cleanup must be driven by user intent (explicit delete/reindex), never inferred from a missing file: url/note materials use the item id as a virtual `relative_path` with no on-disk file, and migrated urls have no snapshot until their first reindex. (The knowledge_item exit-path decision below pulls url materialization into PR B; once landed, only notes keep a virtual path.)
...
- **knowledge_item exit path (decided 2026-06-11)**: `knowledge_item` does not survive into v2.x — the base folder becomes self-describing, `index.sqlite` stays purely derived, each remaining duty of the table gets a new home, then the table drops. Shape validated against QMD (MinerU-Document-Explorer): collection config outside the DB, path-addressed `documents` + hash-addressed `content`/`content_vectors`, scan-sync with an `active` soft-delete — and no item table at all. Sub-decisions:
  - **Derivation invariant**: `index.sqlite` must be rebuildable from "base folder ∪ sources declared in `.cherry/manifest.json`". Provenance lives in the folder itself: app-written md files (url snapshots now, notes at exit time) carry a `cherry`-namespaced frontmatter block (`type`, `source`, `captured_at`/`origin`); base-level intent lives in the manifest.
  - **Marker-gated frontmatter strip**: normalization strips only a leading frontmatter block carrying the `cherry` key — user files' own frontmatter indexes as-is. The marker doubles as the scan-time classifier (url snapshot vs plain md vs note), and the exact strip round-trip (file text → `content.text`) keeps `content_hash` stable, so materializing a file never re-embeds.
  - **Frontmatter `type` names the artifact kind, never the provider**: the vocabulary is materialization shapes (`url-snapshot` now; `note` / `remote-doc` when their writers land), because `type` selects render and refresh semantics (web re-fetch vs provider API re-sync). Brand goes in a `provider` field on `remote-doc` (plus `remote_id` / `revision` for incremental sync) — a `type: feishu`-style vocabulary would force every consumer to enumerate brands whose behaviour is identical. Per demand-first, each type ships with its first writer; the strip stays type-agnostic (gated on the `cherry` key alone), so new types need no normalization change.
  - **Manifest owns what has no file**: pending/failed async fetches and remote-source connections, each with a per-source persistence mode — materialize (url snapshots: the web is volatile), index-only (permission-gated sources like Feishu wikis: chunks+vectors locally, no file; `material.origin` gains `'remote'` later — the CHECK is additive by design), or federated (no local copy at all, query-time remote search). Credentials never enter the manifest (the folder is copyable) — only an account ref; secrets stay in Preference/keychain. Material-level failures already live on material rows (`last_error_*`); the manifest only covers the no-row-yet stage.
  - **Path is identity**: jobs address `(baseId, relativePath)` or a manifest entry id; rename = old row `missing` → GC + new row, chunks/vectors reused via `content_hash`; `material_id` degrades to a row-local surrogate. Per-file ignore rules belong in manifest patterns (QMD `ignore_patterns`-style), not row state — rename-pairing for id continuity stays a future additive optimization.
  - **Guardrails until the exit**: new UI features must not deepen `useQuery(knowledge_item)` coupling; the v2.x list is composed in main (directory scan ⊕ manifest ⊕ index store) and pushed over IPC; cross-base item queries are out, permanently.
  - **One-shot exit migration (v2.x)**: notes materialize to md + frontmatter (hash round-trip, zero re-embeds); directory tombstones and file-less failed rows become manifest entries (the amber state survives; repair = the standard add-directory copy-into-base flow — directories are one-shot copies, not connection-type sources); v2-native file/url/directory rows need nothing; then `DROP TABLE knowledge_item`. No dual-read window, no lazy per-base migration.
  - **Pulled forward into PR B (v2.0)**: (i) live url snapshot capture writes the `cherry` frontmatter from day one; (ii) the v1→v2 migrator materializes migrated urls from `content.text` (frontmatter `origin: v1-migration` — honest about being reconstructed, not freshly fetched; idempotent naming on re-run; real `relative_path` replacing the `item.id` virtual-path convention for urls). v2.x then touches nothing url-shaped.
