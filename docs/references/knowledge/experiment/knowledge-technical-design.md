...
Notes are now materialized like URLs: the add-data-source dialog's note picker copies a selected note's content into a `raw/{slug}.md` snapshot on first index, and `knowledge_item.data` keeps `content` as command input plus the main-written `relativePath`. Unlike a URL snapshot, a note snapshot is written **verbatim with no cherry frontmatter**, so the file round-trips byte-for-byte (the content-hash reuse path).
...
- **PR B** (landed, see §1): migrator writes the final layout (replacing the transitional legacy-remnant detection at store open), URL and note `.md` snapshots (notes verbatim, no frontmatter), conflict "keep copy" (auto-rename with a `_N` suffix), restore copies processed md + URL snapshot, orphan embedding/content GC + rebuild coverage guard. (A chunker-change rebuild trigger is no longer pre-wired via a stored `chunker_config_hash` snapshot — that column was dropped with the §4.1 trim; re-add the detection mechanism if/when the feature lands.)
